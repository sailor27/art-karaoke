...
describe('Karaoke App', () => {
  ...
  describe('lyricChangeReducer', () => {
    ...
    it('Should update state when API lyrics are being requested.', () => {
      const action = actions.requestSong('I\'ve Got a Feeling I\'m Falling');
      const newStateEntry = {
        isFetching: true,
        title: action.title,
  ...
  describe('imageChangeReducer', () => {
    ...
    it('Should change image', () => {
      ...
      expect(imageChangeReducer(initialState, actions.changeImage(testUrl))).toEqual({url:'https://media.giphy.com/media/brwknFAZxzfRm/giphy.gif'});
    });
